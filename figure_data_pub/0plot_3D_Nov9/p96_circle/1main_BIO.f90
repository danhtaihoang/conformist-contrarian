...
! Personal site: https://sites.google.com/site/hoangdanhtai
!-----------------------------------------------------------------------------------------!
...
!!! 2014.11.04: Bo sung histogram 3D (tai cac thoi gian khac nhau)
!!! 2014.11.06: Tach rieng histogram a va b, bo sung phi_a_min, phi_a_max,...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%! 
   PROGRAM main_BIO
   IMPLICIT NONE
   ...
   CHARACTER (256)    :: Ligne20,Ligne21,Ligne22,Ligne23,Ligne40,Ligne41,Ligne42,Ligne43
   ...
   INTEGER (KIND=8),PARAMETER :: n_histo=51
    
   INTEGER (KIND=8) :: i_histo,time_loop,i_histo_a,i_histo_b
   REAL    (KIND=8) :: del_histo
   REAL (KIND=8),DIMENSION(0:n_histo) :: S_histo,H1_histo,H2_histo

   INTEGER (KIND=8),PARAMETER :: n_histo_a=51,n_histo_b=51
   REAL (KIND=8) :: del_histo_a,del_histo_b,phi_a_min,phi_a_max,phi_b_min,phi_b_max   
   REAL (KIND=8),DIMENSION(0:n_histo_a) :: Ha_histo,Sa_histo
   REAL (KIND=8),DIMENSION(0:n_histo_b) :: Hb_histo,Sb_histo
...
!!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!!! 2014.11.04: Histogram 3D2 ==========================================================
!!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!!!  SUBROUTINE histogram_3D2()
!!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   SUBROUTINE histogram_3D2()
   IMPLICIT NONE
        
   WRITE(*,*)'Histogram 3D'
   OPEN(unit=41,file='histogram_3D.dat')
   
   DO time_loop=0,200      
      IF (mod(time_loop,10)==0) THEN   
      H1_histo(:)=0. ; H2_histo(:)=0.
      CALL histogram()
      DO i_histo=1,n_histo
         H1_histo(i_histo)=H1_histo(i_histo)/number_a
         H2_histo(i_histo)=H2_histo(i_histo)/number_b     
         S_histo(i_histo)=(i_histo-0.5)*del_histo
         WRITE(Ligne41,*)S_histo(i_histo),time_loop/100.,H1_histo(i_histo),H2_histo(i_histo)
   ...
   END SUBROUTINE histogram_3D2 

!!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%      
!!! 2014.11.06: Histogram a and b reparately ==========================================
!!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!!!  SUBROUTINE histogram_3D()
!!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   SUBROUTINE histogram_3D()
   IMPLICIT NONE
        
   WRITE(*,*)'Histogram a-b'
   OPEN(unit=42,file='histogram_a.dat')
   OPEN(unit=43,file='histogram_b.dat')
   
   phi_a_min=0. ; phi_a_max=2.*pi
   phi_b_min=0. ; phi_b_max=2.*pi
   del_histo_a = (phi_a_max-phi_a_min)/(n_histo_a-1)
   del_histo_b = (phi_b_max-phi_b_min)/(n_histo_b-1) 
   ...
   DO time_loop=0,3100
      ...
      DO i_histo_a=1,n_histo_a
         Ha_histo(i_histo_a)=Ha_histo(i_histo_a)/number_a 
         Sa_histo(i_histo_a)=phi_a_min+(i_histo_a-0.5)*del_histo_a
         
         !IF (Sa_histo(i_histo_a)<=(pi+pi/50.+pi*time_loop/50000.)) THEN
         IF (Sa_histo(i_histo_a)<(pi+pi/50.)) THEN
         WRITE(Ligne42,*)Sa_histo(i_histo_a),time_loop/100.,Ha_histo(i_histo_a)
         WRITE(42,'(a)') trim(Ligne42)
         END IF
      ...
      DO i_histo_b=1,n_histo_b
         Hb_histo(i_histo_b)=Hb_histo(i_histo_b)/number_b     
         Sb_histo(i_histo_b)=phi_b_min+(i_histo_b-0.5)*del_histo_b
         
         !IF (Sb_histo(i_histo_b)>=(pi-pi/50.+pi*time_loop/100000.)) THEN
         IF (Sb_histo(i_histo_b)>(pi-pi/50.)) THEN
         WRITE(Ligne43,*)Sb_histo(i_histo_b),time_loop/100.,Hb_histo(i_histo_b)
         WRITE(43,'(a)') trim(Ligne43)
         END IF
      ENDDO
      WRITE(Ligne43,*)
      WRITE(43,'(a)') trim(Ligne43)
...
!!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!!!  SUBROUTINE histogram_ab()
!!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   SUBROUTINE histogram_ab()
   IMPLICIT NONE
   
   Ha_histo(:)=0. ; Hb_histo(:)=0.
      
   DO i=1,natom
   
      S2(i)=S(i)-int(S(i)/2./pi)*2.*pi 
      IF (S2(i)<0.) THEN
         S2(i)=S2(i)+2.*pi
      END IF

      IF (Ks(i)>0.9) THEN
         i_histo_a=int((S2(i)-phi_a_min)/del_histo_a)+1      
         IF ((phi_a_min <= S2(i)).and.(S2(i) <= phi_a_max)) THEN
            Ha_histo(i_histo_a)=Ha_histo(i_histo_a)+1.
         END IF
      ELSE
         i_histo_b=int((S2(i)-phi_b_min)/del_histo_b)+1
         IF ((phi_b_min <= S2(i)).and.(S2(i) <= phi_b_max)) THEN
            Hb_histo(i_histo_b)=Hb_histo(i_histo_b)+1.
         END IF
      END IF
      
   END DO
   
   END SUBROUTINE histogram_ab
